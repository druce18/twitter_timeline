...
object TwitterAuthUtil {
    ...
    fun generateAuthHeader(
        ...
        val params = mutableMapOf(
                ...
                OAUTH_NONCE to getNonce(timestamp).toString(),
        ...
        val queryParams = extractQueryParams(baseUrl)
        ...
        val baseString = createSignatureBaseString(methodHttp, url, collectParametersString(params))

        val customerSecret = AuthTempKeys.TWITTER_OAUTH_CONSUMER_SECRET
        val oauthTokenSecret = AuthTempKeys.TWITTER_OAUTH_ACCESS_TOKEN_SECRET
        val signature = createSignature(baseString, createSigningKey(customerSecret, oauthTokenSecret))

        params[OAUTH_SIGNATURE] = signature
        val authHeader = generateAuthString(
    ...
    private fun collectParametersString(params: Map<String, String>): String {
        ...
        for ((key, value) in params) {
            encodedParams.put(percentEncode(key), percentEncode(value))
        }
    ...
    private fun createSigningKey(customerSecret: String, oauthSecret: String?): String {
        val sb = StringBuilder()
        sb.append(percentEncode(customerSecret)).append('&')
        oauthSecret?.let {
            sb.append(percentEncode(oauthSecret))
        }
        return sb.toString()
    }
